...
function Homepage() {
  ...
  useEffect(() => {
    ...
    const backendURI = `http://localhost:3001/api/documents/${userId}`
    axios
      .get(backendURI)
      .then((response) => {
  ...
  const handleCreateDocument = async () => {
    try {
      if (newDocumentName.trim() !== '') {
        const backendURI = `http://localhost:3001/api/create-document/${userId}`;
  ...
  const handleDeleteDocument = async (documentId) => {
    try {
      ...
      if (confirmDelete) {
        const backendURI = `http://localhost:3001/api/delete-document/${documentId}`;
  ...
  const handleUpdateDocumentName = async () => {
    try {
      if (editedDocumentName.trim() !== '') {
        const backendURI = `http://localhost:3001/api/update-document/${selectedDocumentId}`;
